...
const MiniQuiz: React.FC = () => {
  const [miniQuizes, setMiniQuizes] = useState<MiniQuiz[]>(() =>
    miniQuizesData.map((miniQuiz) => ({
      id: miniQuiz.id,
      grades: miniQuiz.grades.map((grade) => ({ ...grade })),
    }))
  );
  const [editing, setEditing] = useState<{
    [miniQuizIdx: number]: { [gradeIdx: number]: boolean };
  }>({});
  const [draftValues, setDraftValues] = useState<{
    [miniQuizIdx: number]: { [gradeIdx: number]: string };
  }>({});
  ...
  const getKey = (miniQuizIdx: number, gradeIdx: number) =>
    `${miniQuizIdx}-${gradeIdx}`;
  ...
  const handleInput = (
    ...
    setDraftValues((prev) => ({
      ...prev,
      [miniQuizIdx]: { ...prev[miniQuizIdx], [gradeIdx]: finalValue },
    }));
  };
  ...
  const handlePaste = (
    ...
    const ref = editableRefs.current[getKey(miniQuizIdx, gradeIdx)];
  ...
  const cancelEdit = (miniQuizIdx: number, gradeIdx: number) => {
    setEditing((prev) => ({
      ...prev,
      [miniQuizIdx]: { ...prev[miniQuizIdx], [gradeIdx]: false },
    }));
    const ref = editableRefs.current[getKey(miniQuizIdx, gradeIdx)];
    if (ref) ref.textContent = miniQuizes[miniQuizIdx].grades[gradeIdx].grade;
    setDraftValues((prev) => ({
      ...prev,
      [miniQuizIdx]: {
        ...prev[miniQuizIdx],
        [gradeIdx]: miniQuizes[miniQuizIdx].grades[gradeIdx].grade,
      },
    }));
  };

  const startEdit = (miniQuizIdx: number, gradeIdx: number) => {
    const current = miniQuizes[miniQuizIdx].grades[gradeIdx].grade;
    setDraftValues((prev) => ({
      ...prev,
      [miniQuizIdx]: { ...prev[miniQuizIdx], [gradeIdx]: current },
    }));
    setEditing((prev) => ({
      ...prev,
      [miniQuizIdx]: { ...prev[miniQuizIdx], [gradeIdx]: true },
    }));
    setTimeout(() => {
      const ref = editableRefs.current[getKey(miniQuizIdx, gradeIdx)];
  ...
  const saveEdit = (labIdx: number, gradeIdx: number) => {
    ...
    const updatedLabs = [...miniQuizes];
    updatedLabs[labIdx].grades[gradeIdx].grade = newValue;
    setMiniQuizes(updatedLabs);
  ...
  return (
    <div className="animate-fade-in transition-all duration-300">
      {miniQuizes.map((miniQuiz, miniQuizIdx) => (
        <div key={miniQuizIdx} className="pb-2">
          <h1 className="font-josefin-sans text-h4 font-semibold mb-4">
            Mini Quiz {miniQuiz.id}
          </h1>
          {miniQuiz.grades.map((grade, gradeIdx) => {
            const isEdit = editing[miniQuizIdx]?.[gradeIdx] || false;
            const value = miniQuizes[miniQuizIdx].grades[gradeIdx].grade;
            const draft = draftValues[miniQuizIdx]?.[gradeIdx] || "";
                ...
                >
                  {isEdit ? (
                    <>
                      <div
                        ref={(el) => {
                          editableRefs.current[getKey(miniQuizIdx, gradeIdx)] =
                            el;
                        }}
                        contentEditable
                        ...
                        onInput={(e) => handleInput(miniQuizIdx, gradeIdx, e)}
                        onKeyDown={handleKeyDown}
                        onPaste={(e) => handlePaste(miniQuizIdx, gradeIdx, e)}
                        style={{ lineHeight: "normal" }}
                      />
                      <Button
                        className="p-2 bg-[#d536364d]/80 hover:bg-[#d536364d] active:bg-[#d536368a]"
                        onClick={() => cancelEdit(miniQuizIdx, gradeIdx)}
                      >
                        <Image
                          src="/close.svg"
                          alt="Cancel"
                  ...
                  <Button
                    ...
                    onClick={() =>
                      isEdit
                        ? saveEdit(miniQuizIdx, gradeIdx)
                        : startEdit(miniQuizIdx, gradeIdx)
                    }
                    disabled={grade.submissionUrl === ""}
                  >
